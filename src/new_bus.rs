...
// Impl this for u8 and u16, and use that to do the generic frontend for Bus
trait NesPrimitive {}
...
impl Addressable for Palettes {
    fn read(&self, addr: u16) -> u8 {
        let addr = self.map_addr(addr) as usize;
        self.0[addr]
    }
    fn write(&mut self, addr: u16, v: u8) {
        let addr = self.map_addr(addr) as usize;
...
impl Bus {
    fn new(mapper: impl Mapper + 'static) -> Self {
        Bus {
            cpu: Vec::new(),
            ppu: Vec::new(),
    ...
    fn cpu_read(&self, addr: u16) -> u8 {
        let device = self.cpu.iter().find(|d| d.range.contains(&addr));
        let addr = self.mapper.map_cpu_read(addr);
        device
            .expect(&format!(
    ...
    fn cpu_write(&mut self, addr: u16, v: u8) {
        let device = self.cpu.iter_mut().find(|d| d.range.contains(&addr));
        let addr = self.mapper.map_cpu_write(addr);
        device
            .expect(&format!(
                "addr {:04X} has not been mapped on CPU bus!",
                addr
    ...
    fn ppu_read(&self, addr: u16) -> u8 {
        let device = self.ppu.iter().find(|d| d.range.contains(&addr));
        let addr = self.mapper.map_ppu_read(addr);
        device
            .expect(&format!(
    ...
    fn ppu_write(&mut self, addr: u16, v: u8) {
        let device = self.ppu.iter_mut().find(|d| d.range.contains(&addr));
        let addr = self.mapper.map_ppu_write(addr);
        device
            .expect(&format!(
                "addr {:04X} has not been mapped on PPU bus!",
                addr
...
fn main() {
    const BANKS: usize = 2;
    let mut bus = Bus::new(Mapper000 {
        mirroring: Mirroring::Vertical,
        prg_banks: BANKS as u8,
    });
    ...
    bus.cpu_connect(AddressSpace::new(0x8000..=0xffff, vec![0; BANKS * 16_384]));
    ...
    bus.ppu_write(addr1, 12);
    println!("{}", bus.ppu_read(addr2));
}
